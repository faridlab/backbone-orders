...
```bash
# Generate everything
backbone schema generate orders --target all

# Or generate specific targets
backbone schema generate orders --target proto,rust,sql
backbone schema generate orders --target handler,grpc,cqrs
backbone schema generate orders --target repository,events
```
...
```rust
use backbone_orders::OrdersModule;

// Create module instance with builder pattern
let module = OrdersModule::builder()
    .with_database(pool)
    .build()?;
...
2. Run generator: `backbone schema generate orders --target all`
...
2. Generate migration: `backbone migration alter {Entity} orders -d "description"`
3. Regenerate code: `backbone schema generate orders --target all`
4. Run migrations
...
```bash
# Run all tests
cargo test --package backbone-orders

# Run with database
DATABASE_URL=postgresql://... cargo test --package backbone-orders
```
